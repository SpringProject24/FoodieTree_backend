...
const modal = document.getElementById('store-calendar-modal');
...
function showModal(year, month, day) {
    ...
    modalDetailsElement.textContent = `Selected date: ${dateString}`;
...
async function openModal(reservationId) {
    ...
    $modalDetails.innerHTML = `
        <div class="reservation-detail-item" data-reservation-id="${reservationId}">
            <p>픽업하는 손님의 닉네임: ${reservation.nickname}</p>
            <p>상태: ${reservation.status}</p>
            <p>${reservation.statusInfo}</p>
            <img src="${reservation.profileImage}" alt="profile Image"/>
            ${reservation.status === 'RESERVED' ? '<div>${reservation.nickname}님이 픽업하러 오는 중</div>' : '<div>상태 확인 중</div>'}
        </div>
    `;
